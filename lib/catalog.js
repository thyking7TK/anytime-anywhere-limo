...
export async function getCatalog(options = {}) {
  ...
  const vehicleSet = includeInactive ? vehicles : vehicles.length ? vehicles : defaultVehicles;
...
export async function saveCatalog({ vehicles, pricingMatrix, pricingSettings }) {
  ...
  for (const service of services) {
    for (const vehicle of normalizedVehicles) {
      pricingPairs.push([service.id, vehicle.slug]);
    }
  }
  ...
  await sql.begin(async (transaction) => {
    await transaction`
      ...
      WHERE vehicle_slug NOT IN ${transaction(vehicleSlugs)}
    `;

    await transaction`
      DELETE FROM vehicles
      WHERE slug NOT IN ${transaction(vehicleSlugs)}
    `;
    ...
    await transaction`
      DELETE FROM pricing_rules
      WHERE (service_id, vehicle_slug) NOT IN ${transaction(pricingPairs)}
    `;
