...
async function seedDefaultCatalog(sql) {
  ...
  // Force-apply mileageRate to $2.50/mile — overrides any stale DB value.
  await sql`
    INSERT INTO pricing_settings (key, value_text)
    VALUES ('mileageRate', '2.5')
    ON CONFLICT (key) DO UPDATE SET value_text = '2.5'
  `;

  // Remove minimum quote — no floor on trip pricing.
  await sql`
    INSERT INTO pricing_settings (key, value_text)
    VALUES ('minimumQuote', '0')
    ON CONFLICT (key) DO UPDATE SET value_text = '0'
  `;
}
