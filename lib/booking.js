...
export { getMaineTourById, getMaineTourPricingOption, maineTourPackages };
...
export const defaultForm = {
  ...
  tourPackageId: maineTourPackages[0]?.id ?? "",
...
export function normalizeBookingForm(input = {}) {
  return {
    ...
    tourPackageId: normalizeString(
      input.tourPackageId,
      defaultForm.tourPackageId,
    ),
    tourPricingTierIndex: normalizeString(
      input.tourPricingTierIndex,
      defaultForm.tourPricingTierIndex,
    ),
  };
}
...
// ── US Federal + Maine & Massachusetts Holiday detection ────────────────────
// Returns the nth weekday of a given month/year (e.g. 3rd Monday of January).
function nthWeekday(year, month, weekday, n) {
  ...
  return day <= 31 ? day : null; // guard against overflow
}

// Returns the last weekday of a given month/year.
...
// Observed date when a fixed holiday falls on a weekend:
// Saturday → Friday, Sunday → Monday.
function observedDate(year, month, day) {
  const d = new Date(Date.UTC(year, month - 1, day));
  const dow = d.getUTCDay();
  if (dow === 6) return { month, day: day - 1 }; // Saturday → Friday
  if (dow === 0) return { month, day: day + 1 }; // Sunday  → Monday
  return { month, day };
}

export function isUSHoliday(dateInput) {
  ...
  for (const h of fixed) {
    const obs = observedDate(y, h.month, h.day);
    if (obs.month === m && obs.day === d) return true;
  }

  // New Year's Day observed on Dec 31 when Jan 1 is Saturday
  const newYearObs = observedDate(y, 1, 1);
  if (newYearObs.month === 1 && newYearObs.day === 2) {
    // Jan 1 is Sunday → observed Monday Jan 2; also check Dec 31 of prev year
    const dec31 = observedDate(y, 1, 1);
    if (m === 12 && d === 31 && new Date(Date.UTC(y, 0, 1)).getUTCDay() === 6) return true;
  }

  // Floating holidays
  const floating = [
    { month: 1,  weekday: 1, n: 3, label: "MLK Day"        }, // 3rd Mon Jan
    { month: 2,  weekday: 1, n: 3, label: "Presidents Day"  }, // 3rd Mon Feb
    { month: 9,  weekday: 1, n: 1, label: "Labor Day"       }, // 1st Mon Sep
    { month: 10, weekday: 1, n: 2, label: "Columbus Day"    }, // 2nd Mon Oct
    { month: 11, weekday: 4, n: 4, label: "Thanksgiving"    }, // 4th Thu Nov
  ];

  for (const h of floating) {
    const day = nthWeekday(y, h.month, h.weekday, h.n);
  ...
  // ── Maine & Massachusetts state holidays ──────────────────────────────────

  // Patriots' Day — 3rd Monday of April (ME & MA)
  if (m === 4 && d === nthWeekday(y, 4, 1, 3)) return true;

  // Maine Patriot's Day / Fast Day (ME only) — last Monday of April
  // Note: Maine now uses the 3rd Monday (same as MA Patriots' Day above).

  // Town Meeting Day / Evacuation Day — March 17, observed (Suffolk Co. MA)
  // Statewide in MA since 1941 as a legal holiday for Boston area.
  const evacObs = observedDate(y, 3, 17);
  if (evacObs.month === m && evacObs.day === d) return true;

  // Bunker Hill Day — June 17, observed (Suffolk Co. MA)
  const bunkerObs = observedDate(y, 6, 17);
  if (bunkerObs.month === m && bunkerObs.day === d) return true;
...
export function calculateCustomQuote(
  ...
  // Only miles are required — pricing is mileage-based. Hours contribute a
  // time charge when provided but are not required to produce a quote.
  if (!estimatedMiles) {
    return createBreakdown({
      quoteMode: "request",
      pricingModel: "custom",
      ...
      lineItems: [],
      note:
        "Enter a destination to calculate the mileage-based quote. Otherwise, submit the request and Autovise will quote it manually.",
      details: {},
    });
  }
  ...
  const extraStops = Math.max(0, roundMoney(options.extraStops));
  const waitHours = Math.max(0, coerceNumber(options.waitHours, 0));
  const timeCharge = estimatedHours > 0 ? roundMoney(estimatedHours * settings.customHourlyBasis) : 0;
  ...
  const lineItems = [
    ...(timeCharge > 0 ? [{
      key: "time-charge",
      label: `Time charge (${estimatedHours} x ${formatCurrency(settings.customHourlyBasis)})`,
      amount: timeCharge,
    }] : []),
    {
      key: "mileage-charge",
...
export function calculateMaineTourQuote(tourId, tierIndex = 0) {
  const tour = getMaineTourById(tourId);
  const option = getMaineTourPricingOption(tourId, Number(tierIndex));
  const total = parseTourPrice(option?.price);
  const lineItems = total
    ? [
        {
          key: "tour-package",
          label: `${tour.title} - ${option.title}`,
          amount: total,
        },
      ]
    : [];
  ...
  return createBreakdown({
    quoteMode: total > 0 ? "instant" : "request",
    pricingModel: "maine-tours",
    total,
    lineItems,
    note:
      "Final pricing may be adjusted for route changes, additional scenic stops, excess wait time, special requests, and custom itinerary changes.",
    details: {
...
export function calculateEstimate(formInput, catalog = {}) {
  ...
  // Auto-detect US federal holidays from the booking date
  const holiday = form.date ? isUSHoliday(new Date(form.date)) : false;
  ...
  switch (form.service) {
    ...
    case "maine-tours":
      return calculateMaineTourQuote(
        form.tourPackageId,
...
export function validateBooking(formInput, options = {}) {
  ...
  if (form.service !== "maine-tours" && options.catalog && !vehicle) {
  ...
  if (form.service === "custom") {
    ...
    // Hours are optional — auto-populated from the distance API.
    // Miles are required to compute a mileage-based quote.
    if (!Number.isFinite(estimatedTripMiles) || estimatedTripMiles <= 0) {
      nextErrors.estimatedTripMiles =
        "Enter a pickup and destination so the distance can be calculated.";
    }
  ...
  if (form.service === "maine-tours") {
    if (!selectedTour) {
      nextErrors.tourPackageId = "Choose a valid Maine touring package.";
    }

    if (!selectedTourPricing) {
      nextErrors.tourPricingTierIndex = "Choose a valid package tier.";
    }
  }
