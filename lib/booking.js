...
export function getCatalogVehicles(catalog = {}) {
  return catalog.vehicles?.length ? catalog.vehicles : getDefaultCatalog().vehicles;
}
...
export function calculateEstimate(formInput, catalog = {}) {
  ...
  const vehicle = getVehicleBySlug(form.vehicle, { vehicles }) ?? vehicles[0];
  const baseRate = pricingMatrix?.[form.service]?.[vehicle.slug] ?? 0;
