...
function resolveWhen(booking) {
  if (booking.when) {
    return booking.when;
  }

  return formatRideLocalTimestamp(booking.rideLocalAt);
}

function resolveReturnWhen(booking) {
  if (booking.returnWhen) {
    return booking.returnWhen;
  }

  return formatRideLocalTimestamp(booking.returnLocalAt);
}

function resolveServiceTitle(booking) {
  return (
...
function buildBookingSummary(booking) {
  ...
  const when = resolveWhen(booking);
  const returnWhen = resolveReturnWhen(booking);
  const total = formatCurrency(Number(booking.estimate?.total ?? 0));
...
function buildPaymentCustomerHtml(booking) {
  const summary = buildBookingSummary(booking);
  const firstName = getFirstName(booking.fullName);
  const paymentAmount = formatCurrency(
    Number(booking.paymentAmount ?? booking.estimatedDeposit ?? booking.estimate?.total ?? 0),
  );
    ...
    <div style="font-family:Inter,Arial,sans-serif;background:#07090d;color:#f7f1e8;padding:24px;">
      <div style="max-width:680px;margin:0 auto;border:1px solid rgba(210,176,107,0.16);border-radius:24px;overflow:hidden;background:linear-gradient(180deg,#11141b 0%,#0b0d12 100%);box-shadow:0 24px 80px rgba(0,0,0,0.38);">
        ...
        <div style="padding:30px 32px 34px;">
          ...
          <p style="margin:0 0 12px;font-size:15px;line-height:1.9;color:#d2cec7;">
            We've received your payment for booking ${escapeHtml(booking.reference)}.
          </p>
          <p style="margin:0 0 24px;font-size:15px;line-height:1.9;color:#d2cec7;">
            Your card payment of <strong style="color:#f7f1e8;">${escapeHtml(paymentAmount)}</strong> has been successfully processed. Our team will continue with final scheduling and ride confirmation.
          </p>
          ...
          <div style="border:1px solid rgba(210,176,107,0.16);border-radius:20px;background:rgba(255,255,255,0.02);padding:22px 22px 18px;">
            ...
            <table style="width:100%;border-collapse:collapse;font-size:14px;">
              <tr><td style="padding:8px 0;color:#a6a29b;">Reference</td><td style="padding:8px 0;">${escapeHtml(booking.reference)}</td></tr>
              <tr><td style="padding:8px 0;color:#a6a29b;">Service</td><td style="padding:8px 0;">${escapeHtml(summary.serviceTitle)}</td></tr>
              <tr><td style="padding:8px 0;color:#a6a29b;">Pickup</td><td style="padding:8px 0;">${escapeHtml(booking.pickup)}</td></tr>
              <tr><td style="padding:8px 0;color:#a6a29b;">Drop-off</td><td style="padding:8px 0;">${escapeHtml(booking.dropoff)}</td></tr>
              <tr><td style="padding:8px 0;color:#a6a29b;">When</td><td style="padding:8px 0;">${escapeHtml(summary.when)}</td></tr>
              <tr><td style="padding:8px 0;color:#a6a29b;">Payment received</td><td style="padding:8px 0;">${escapeHtml(paymentAmount)}</td></tr>
            </table>
          </div>
          ...
          <div style="margin-top:24px;padding:22px;border:1px solid rgba(255,255,255,0.06);border-radius:20px;background:linear-gradient(180deg,rgba(255,255,255,0.02),rgba(255,255,255,0.01));">
            ...
            <ul style="margin:0;padding-left:18px;color:#d2cec7;line-height:1.9;">
              <li>Your payment is secured and recorded</li>
              <li>Our team will confirm final scheduling shortly</li>
              <li>If any details still need clarification, we'll contact you directly</li>
            </ul>
          </div>
          ...
          <div style="margin-top:24px;padding:22px;border-radius:20px;background:rgba(210,176,107,0.08);border:1px solid rgba(210,176,107,0.18);">
            ...
            <p style="margin:0;color:#d2cec7;line-height:1.8;">
              For urgent changes or support, you can reach us directly:
            </p>
            <p style="margin:16px 0 0;">
              <a href="tel:+12078803733" style="display:inline-block;border-radius:999px;background:#d2b06b;color:#121317;text-decoration:none;font-weight:700;padding:12px 18px;">
                Call or Text: +1 (207) 880-3733
...
function buildPaymentCustomerText(booking) {
  const summary = buildBookingSummary(booking);
  const firstName = getFirstName(booking.fullName);
  const paymentAmount = formatCurrency(
    Number(booking.paymentAmount ?? booking.estimatedDeposit ?? booking.estimate?.total ?? 0),
  ...
  return [
    ...
    `Hello ${firstName},`,
    "",
    `We've received your payment for booking ${booking.reference}.`,
    "",
    ...
    "",
    "What Happens Next",
    "Your payment is secured and recorded",
    "Our team will confirm final scheduling shortly",
    "If any details still need clarification, we'll contact you directly",
    "",
    "Need Immediate Assistance?",
    "Call or Text: +1 (207) 880-3733",
    "",
...
function buildPaymentAdminHtml(booking) {
  const summary = buildBookingSummary(booking);
  const paymentAmount = formatCurrency(
    Number(booking.paymentAmount ?? booking.estimatedDeposit ?? booking.estimate?.total ?? 0),
  );
  ...
  return `
    <div style="font-family:Arial,sans-serif;background:#0b0d11;color:#f7f1e8;padding:24px;">
      <div style="max-width:680px;margin:0 auto;border:1px solid rgba(255,255,255,0.08);border-radius:20px;padding:28px;background:#14171d;">
        ...
        <h1 style="font-size:34px;line-height:1.1;margin:0 0 12px;">${escapeHtml(booking.reference)}</h1>
        <p style="margin:0 0 24px;color:#d2cec7;">
          ${escapeHtml(booking.fullName)} completed payment for ${escapeHtml(summary.serviceTitle.toLowerCase())}.
        </p>
        <table style="width:100%;border-collapse:collapse;font-size:14px;">
          <tr><td style="padding:8px 0;color:#a6a29b;">Passenger</td><td style="padding:8px 0;">${escapeHtml(booking.fullName)}</td></tr>
          <tr><td style="padding:8px 0;color:#a6a29b;">Email</td><td style="padding:8px 0;">${escapeHtml(booking.email)}</td></tr>
          ...
          ${
            booking.stripeCheckoutSessionId
              ? `<tr><td style="padding:8px 0;color:#a6a29b;">Checkout session</td><td style="padding:8px 0;">${escapeHtml(booking.stripeCheckoutSessionId)}</td></tr>`
              : ""
          }
        </table>
      </div>
...
function buildPaymentAdminText(booking) {
  const summary = buildBookingSummary(booking);
  const paymentAmount = formatCurrency(
    Number(booking.paymentAmount ?? booking.estimatedDeposit ?? booking.estimate?.total ?? 0),
  );
...
export async function sendPaymentCompleteEmails(booking) {
  const config = getEmailConfig();
  ...
  if (config.notificationEmail) {
    tasks.push(
      resend.emails.send({
        ...
        html: buildPaymentAdminHtml(booking),
        text: buildPaymentAdminText(booking),
        replyTo: booking.email,
      }),
    );
  }
  ...
  tasks.push(
    resend.emails.send({
      ...
      to: booking.email,
      subject: "Your payment has been received",
      html: buildPaymentCustomerHtml(booking),
      text: buildPaymentCustomerText(booking),
      replyTo: config.replyToEmail || config.notificationEmail || booking.email,
    }),
  );

  const [adminResult, customerResult] = await Promise.allSettled(tasks);
  const sentAdminEmail = Boolean(config.notificationEmail);

  return {
    enabled: true,
    admin: sentAdminEmail
      ? adminResult.status === "fulfilled"
        ? "sent"
        : "failed"
      : "skipped",
    customer:
      (sentAdminEmail ? customerResult : adminResult)?.status === "fulfilled"
        ? "sent"
        : "failed",
  };
}
    ...
    html: `
      <div style="font-family:Arial,sans-serif;background:#0b0d11;color:#f7f1e8;padding:24px;">
        <div style="max-width:680px;margin:0 auto;border:1px solid rgba(255,255,255,0.08);border-radius:20px;padding:28px;background:#14171d;">
          ...
          <h1 style="font-size:32px;line-height:1.1;margin:0 0 12px;">${escapeHtml(inquiry.fullName)}</h1>
          <p style="margin:0 0 24px;color:#d2cec7;">
            A new inquiry was sent through the Autovise contact form.
          </p>
          <table style="width:100%;border-collapse:collapse;font-size:14px;">
            <tr><td style="padding:8px 0;color:#a6a29b;">Name</td><td style="padding:8px 0;">${escapeHtml(inquiry.fullName)}</td></tr>
            <tr><td style="padding:8px 0;color:#a6a29b;">Email</td><td style="padding:8px 0;">${escapeHtml(inquiry.email)}</td></tr>
            <tr><td style="padding:8px 0;color:#a6a29b;">Phone</td><td style="padding:8px 0;">${escapeHtml(inquiry.phone)}</td></tr>
            <tr><td style="padding:8px 0;color:#a6a29b;">Service</td><td style="padding:8px 0;">${escapeHtml(inquiry.service)}</td></tr>
            <tr><td style="padding:8px 0;color:#a6a29b;vertical-align:top;">Message</td><td style="padding:8px 0;">${escapeHtml(inquiry.message)}</td></tr>
          </table>
        </div>
      </div>
    `,
