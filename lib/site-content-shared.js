...
function normalizeEmail(value, fallback = "") {
  ...
  if (!normalized || normalized === "book@autovise.com") {
    return fallback;
  }
...
export function getDefaultSiteContent() {
  return {
    ...
    hero: {
      ...
      bookingTitle: "Plan your ride",
      bookingDescription:
        "Share your route, timing, and service type to receive a luxury transportation estimate for airport transfers, executive travel, long-distance rides, or VIP service.",
      bookingPill: "Nationwide requests",
    },
    bookingUi: {
      successLabel: "Booking request received",
      pricingNote:
        "This estimate is based on service type, vehicle, timing, and passenger count. Long-distance and nationwide trips may require a custom confirmation before dispatch.",
      unavailableMessage:
